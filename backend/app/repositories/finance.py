...
class FinanceRepository:
    ...
    async def get_by_id(self, fee_id: str) -> FeeCollection | None:
        result = await self.db.execute(select(FeeCollection).where(FeeCollection.id == fee_id))
        return result.scalars().first()

    async def get_by_invoice_no(self, invoice_no: str) -> FeeCollection | None:
        result = await self.db.execute(
            select(FeeCollection).where(FeeCollection.invoice_no == invoice_no)
        )
        return result.scalars().first()
    ...
    async def update_payment(
        self, fee: FeeCollection, update_in: FeePaymentUpdate
    ) -> FeeCollection:
